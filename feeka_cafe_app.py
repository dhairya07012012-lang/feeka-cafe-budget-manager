...
"""
            ...
            if st.session_state.advice_history:
                st.markdown("---")
                st.markdown("### 📚 History")
                for item in reversed(st.session_state.advice_history[-3:]):
                    with st.expander(f"{item['question'][:40]}... - {item['time']}"):
                        st.markdown(f"**Q:** {item['question']}")
                        st.markdown(f"**A:** {item['advice']}")
        
        except ImportError:
            st.error("OpenAI library not installed properly")

# Footer
st.markdown("---")
st.markdown(f"""
<div style="text-align: center; opacity: 0.6;">
    <p>🍽️ Feeka Cafe Budget Manager | Theme: {st.session_state.theme.title()}</p>
</div>
""", unsafe_allow_html=True)
